...
function Variables2() {
  ...
  const llantasBaseCompletas = [
    ...
    { id: 6, nombre: 'Llanta Extra Trasera Izquierda', fp: false, pe: false, pa: false, desgaste: false },
    { id: 7, nombre: 'Llanta Trasera Derecha', fp: false, pe: false, pa: false, desgaste: false },
    { id: 8, nombre: 'Llanta Extra Trasera Derecha', fp: false, pe: false, pa: false, desgaste: false }
  ];

    if (tipoVehiculo === 'camion') {
      setLlantasParte1(llantasBaseCompletas.filter(llanta => [1, 2, 5, 6].includes(llanta.id)));
      setLlantasParte2(llantasBaseCompletas.filter(llanta => [7, 8].includes(llanta.id)));
    } else { // Para sedan, pickup y panel
      setLlantasParte1(llantasBaseCompletas.filter(llanta => [1, 2].includes(llanta.id)));
      setLlantasParte2(llantasBaseCompletas.filter(llanta => [5, 7].includes(llanta.id)));
    }
  };
